...
4. Add every sample as a direct function-call check inside the entry-point function of each boilerplate. Compare the returned value with the expected value and print the resulting boolean. Keep these checks commented so they do not affect judging.
...
Include the title, concise description, C++ function signature, input format, output format, constraints, examples, example function tests, and important edge cases. The signature must exactly match the C++ starter, excluding the function body. Keep the statement consistent with the driver files. Do not put the solution implementation in the statement.

Add an `Example Function Tests` section containing commented or standalone C++ boolean checks that call the submitted function directly, for example:

```cpp
cout << (solve({1, 2, 3}) == 6) << '\n';
```

Explain that each check prints `1` when the returned value matches the expected value.
...
Represent each sample as a direct call to the submitted function using the language's normal data syntax. Compare its return value with the expected result and print the boolean, following these patterns:
...
Keep every example check commented so it is runnable when uncommented but cannot add output during judging. Use the same arguments and expected values as the statement examples. Do not create unused `sample_input` or `expected_output` string variables.
...
- Check every boilerplate contains a commented direct function-call check for each statement example inside its entry point.
- Temporarily uncomment the example checks and verify that each comparison evaluates to true without changing the delivered boilerplates.
